...
export default function DigitButton({ theme, dispatch, dchar,cchar }) {
  const btnTheme =
    theme === 'one'
  ...
  return (
    <button
      type="button"
      className={`text-xl button ${btnText} ${btnTheme}`}
      onClick={() => dispatch({ type: ACTIONS.ADD_DIGIT, payload: { dchar,cchar } })}
    >
      {dchar}
    </button>
  );
